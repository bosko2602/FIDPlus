...
function handleMessage(message, target)
{
	...
	else if (message.type == 'teamTraining')
	{
		for (var i in message.dates)
		{
			console.log('setting interval ' + i + ' for ' + message.dates[i].name);
		
			(function()
			{
				var key = i;
				
				intervals[i] = timers.setInterval(function(){ ttnote(key, message.dates[key]); }, 60000);
			})();
		}
	}
}
...
function ttnote(id, info)
{
	var tt = info['date'];
	
	console.log('TT for ' + info['name'] + ' at ' + tt);
	...
	if (tt != '')
	{
		...
		if (now.getTime() >= ttdate.getTime() && now.getTime() < (ttdate.getTime() + 1800000))
		{
			...
			timers.clearInterval(intervals[id]);
			delete intervals[id];
		}
	}
}
